...
export function calculatePricing(courses, comboDeals = [], discountTiers = []) {
  ...
  const hasNullPrice = courses.some(c => c.price === null || c.price === undefined);
  ...
  const selectedCount = courses.length;
  // Normalize IDs to strings for comparison (handles ObjectId vs string mismatch)
  const selectedIds = courses.map(c => String(c._id)).sort();

  // Step 1: Check for exact combo deal match (highest priority) - using MongoDB IDs
  // Note: IDs are normalized to strings here to handle both ObjectId objects and plain strings
  let activeComboDeal = null;
  for (const deal of comboDeals || []) {
    // Guard against missing courseIds and normalize to strings
    // Sort a copy to avoid mutating the caller's data
  ...
  if (activeComboDeal) {
    ...
    const discountedCourses = courses
    ...
    if (activeTier) {
      ...
      discountAmount = subtotal - totalPrice;
      discountSource = 'tier';
      discountReason = `You selected ${selectedCount} course${selectedCount > 1 ? 's' : ''} and qualified for the Volume Discount.`;
    } else {
      totalPrice = subtotal;
      discountSource = 'none';
