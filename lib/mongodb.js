...
function getClientPromise() {
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    // Production: cache at module scope so repeated calls in the same
    // warm serverless instance reuse the same connection instead of
    // opening a new one and re-running seedDatabase() every time.
    if (!clientPromise) {
      client = new MongoClient(uri, options);
